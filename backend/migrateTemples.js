...
const templeBookingSchema = new mongoose.Schema({
  ...
  wish: String,
  date: Date,
  booking_status: Number, // 1: Pending, 2: Confirmed, 3: Cancelled
  payment_type: Number,   // 1: Cash, 2: Online, 3: Card
  payment_status: Number, // 1: Pending, 2: Paid, 3: Failed
  razorpay_order_id: String,
  razorpay_payment_id: String,
  payment_date: Date,
  purchased_member_card_id: Number, // Keep as Number if not migrated yet
...
const TempleBooking = mongoose.models.TempleBooking || mongoose.model("TempleBooking", templeBookingSchema);
  ...
  try {
    ...
    while ((match = regex.exec(sql)) !== null) {
      ...
      for (let row of rows) {
        ...
        const sqlTempleId = Number(values[3]);
        const sqlUserId = Number(values[2]);
        const oldSqlId = Number(values[0]);

        const doc = {
          sql_id: oldSqlId,
          booking_id: values[1],
          // Converting SQL IDs to MongoDB ObjectIDs
    ...
    console.log(`🎉 Success! Migrated ${totalMigrated} Bookings with ObjectID links.`);
    process.exit(0);
  } catch (err) {
